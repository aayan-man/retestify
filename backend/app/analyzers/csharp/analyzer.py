...
_TEST_ATTRIBUTES = {"Fact", "Theory", "Test", "TestCase", "TestMethod"}
...
class CSharpAnalyzer(TreeSitterAnalyzer):
    ...
    decision_node_types = {"if_statement", "for_statement", "while_statement", "do_statement", "foreach_statement", "catch_clause", "switch_section"}
    nesting_node_types = {"if_statement", "for_statement", "while_statement", "do_statement", "foreach_statement", "try_statement", "switch_statement"}
    ...
    def detect(self, workspace: Workspace) -> bool:
        ...
        if next(root.rglob("*.csproj"), None) is not None or next(root.rglob("*.sln"), None) is not None:
            return True
        return next(root.rglob("*.cs"), None) is not None

    def is_test_file(self, file_path: Path) -> bool:
        name = file_path.name
        return name.endswith("Test.cs") or name.endswith("Tests.cs")
    ...
    def _extract_tests(
        ...
        def has_test_attribute(node: Node) -> bool:
            for attr_list in (c for c in node.children if c.type == "attribute_list"):
                for attr in attr_list.named_children:
                    if attr.type != "attribute":
                        continue
                    name_node = attr.child_by_field_name("name")
                    if name_node and self._text(source, name_node) in _TEST_ATTRIBUTES:
                        return True
            return False

        def walk(node: Node) -> None:
            for child in node.named_children:
                if child.type == "method_declaration" and has_test_attribute(child):
                    name_node = child.child_by_field_name("name")
                    name = self._text(source, name_node) if name_node else "unknown"
                    tests.append(
                        TestCase(
