...
_ATTRIBUTE_BY_FRAMEWORK = {"xunit": "Fact", "nunit": "Test", "mstest": "TestMethod"}
...
def target_test_file(workspace: Workspace, component: Component) -> Path:
    ...
    return workspace.source_dir / source_path.parent / f"{class_name}Tests.cs"
...
def write_generated_test(
    ...
    using = "using Xunit;" if framework == "xunit" else ("using NUnit.Framework;" if framework == "nunit" else "using Microsoft.VisualStudio.TestTools.UnitTesting;")
    header = f"{using}\n\npublic class {class_name}\n{{\n"

    code = generated.code.strip()
    attribute = _ATTRIBUTE_BY_FRAMEWORK.get(framework)
    if attribute and f"[{attribute}]" not in code:
        code = f"[{attribute}]\n{code}"
    ...
    return TestCase(
        id=f"cs:{rel_path}:{generated.test_name}:{start_line}",
